...
func Accessor(ctx RuleContext) string {
	if ctx.AccessorOverride != "" {
		return ctx.AccessorOverride
	}
	return fmt.Sprintf("m.Get%s()", replaceProtected(pgsgo.PGGUpperCamelCase(ctx.Field.Name())))
}

func ErrName(m pgs.Message) pgs.Name {
	return m.Name().UpperSnakeCase() + "ValidationError"
}

func ErrIdxCause(ctx RuleContext, idx, cause string, reason ...interface{}) string {
	f := ctx.Field
	n := replaceProtected(pgsgo.PGGUpperCamelCase(f.Name()))
...
func Err(ctx RuleContext, reason ...interface{}) string {
	return ErrIdxCause(ctx, "", "nil", reason...)
}

func ErrCause(ctx RuleContext, cause string, reason ...interface{}) string {
	return ErrIdxCause(ctx, "", cause, reason...)
}

func ErrIdx(ctx RuleContext, idx string, reason ...interface{}) string {
	return ErrIdxCause(ctx, idx, "nil", reason...)
}

func Unwrap(ctx RuleContext, name string) (RuleContext, error) {
